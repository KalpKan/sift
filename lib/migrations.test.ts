...
describe("supabase/migrations/0001_create_schema.sql", () => {
  it("creates the placeholder schema and grants usage to the three API roles", () => {
    expect(first).toContain("create schema if not exists __app__;");
    expect(first).toContain("grant usage on schema __app__ to anon, authenticated, service_role;");
  });

  it("sets default privileges for tables, sequences and functions", () => {
    for (const kind of ["tables", "sequences", "functions"]) {
      expect(first).toContain(
        `alter default privileges in schema __app__ grant all on ${kind} to anon, authenticated, service_role;`,
      );
    }
  });

  it("ships a select-1 health function the /api/health route can call", () => {
    expect(first).toContain("create or replace function __app__.health_select_one()");
    expect(first).toContain("grant execute on function __app__.health_select_one() to anon, authenticated, service_role;");
  });
