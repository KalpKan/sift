...
--
-- BEFORE APPLYING: replace every `sift` below with your schema name (the same
-- value you put in NEXT_PUBLIC_APP_SCHEMA). From the repo root:
--     sed -i '' 's/sift/myapp/g' supabase/migrations/*.sql
-- and check nothing is left:  grep -rn sift supabase/  (must print nothing).
--
...
-- Exposed schemas → add `sift` (keep the existing entries). The API version
...
create schema if not exists sift;

-- The API roles must be able to see the schema at all ...
grant usage on schema sift to anon, authenticated, service_role;
...
alter default privileges in schema sift grant all on tables to anon, authenticated, service_role;
alter default privileges in schema sift grant all on sequences to anon, authenticated, service_role;
alter default privileges in schema sift grant all on functions to anon, authenticated, service_role;
...
create or replace function sift.health_select_one()
returns integer
language sql
stable
as $$ select 1 $$;

grant execute on function sift.health_select_one() to anon, authenticated, service_role;

-- Objects created in this same file are covered explicitly (default privileges
-- only apply to objects created afterwards).
grant all on all tables in schema sift to anon, authenticated, service_role;
grant all on all sequences in schema sift to anon, authenticated, service_role;
